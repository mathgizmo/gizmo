        ...
        <form role="form"
            ...
            <div class="card-body p-0">
                ...
                <div class="form-group row mt-3 {{ $errors->has('description') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-8">
                        <textarea type="text" name="description" id="description" class="form-control">{{ old('description', $level->description) }}</textarea>
                ...
                <div class="form-group row{{ $errors->has('dependency') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-8 radio">
                        <label for="type" class="col-md-3">
                            <input {{ ($level->dependency == true) ? 'checked="checked"' : ''}} type="checkbox" name="dependency" value="1">
                        </label>
                ...
                <div class="form-group row{{ $errors->has('dev_mode') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-8 radio">
                        <label for="type" class="col-md-3">
                            <input {{ ($level->dev_mode == true) ? 'checked="checked"' : ''}} type="checkbox" name="dev_mode" value="1">
                        </label>
                        ...
                        <select class="form-control" name="order_no" id="order_no">
                            ...
                            @if ($total_level > 0)
                                @for($count = 2; $count <= $total_level + 1; $count++)
                                    <option <?php echo ($count == $level->order_no) ? 'selected="selected"' : ''; ?> value="{{$count}}">{{$count}}</option>
                                @endfor
                            @endif
                        </select>
