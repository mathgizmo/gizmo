...
@section('content')
    ...
    <div class="card">
        <div class="card-header font-weight-bold d-flex flex-row">Edit Topic</div>
        <form id="topic" class="form-horizontal" role="form" action="{{ route('topics.update', $topic->id) }}"
              method="POST">
            <div class="card-body p-0">
                            ...
                            @if (count($levels) > 0)
                                ...
                                @foreach($levels as $level)
                                    <option value="{{$level->id}}"
                                            ...
                                            @endif  @if ($level->id == $topic->lid) selected="selected"
                                            @endif
                                    >{{$level->title}}</option>
                                @endforeach
                            @endif
                ...
                <div class="form-group row{{ $errors->has('topic_title') ? ' has-error' : '' }}">
                    <label for="topic_title" class="col-md-2 form-control-label ml-3 font-weight-bold"> Title</label>

                    <div class="col-md-8">
                        <input id="topic_title" type="text" class="form-control" name="topic_title"
                               placeholder="Enter Topic text.." value="{{$topic->title}}">
                ...
                <div class="form-group row{{ $errors->has('short_name') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-8">
                        <input id="short_name" type="text" class="form-control" name="short_name"
                               placeholder="Enter Short Name" value="{{ $topic->short_name }}">
                ...
                <div class="form-group row{{ $errors->has('dependency') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-8 radio">
                        <label for="type" class="col-md-3"> <input
                                    {{ ($topic->dependency == true) ? 'checked="checked"' : ''}} type="checkbox"
                ...
                <div class="form-group row{{ $errors->has('dev_mode') ? ' has-error' : '' }}">
                    <label for="type" class="col-md-2 form-control-label ml-3 font-weight-bold">Topic in
                        development</label>
                    <div class="col-md-8 radio">
                        <label for="type" class="col-md-3"> <input
                                    {{ ($topic->dev_mode == true) ? 'checked="checked"' : ''}} type="checkbox"
                        ...
                        <select class="form-control" name="order_no" id="order_no">
                            ...
                            @if ($total_topic > 0)
                                @for($count = 2; $count <= $total_topic + 1; $count++)
                                    <option
                                        <?php echo ($count == $topic->order_no) ? 'selected="selected"' : ''; ?> value="{{$count}}">{{$count}}</option>
                                @endfor
                            @endif
                        </select>
