...
@section('content')
    <div class="card">
        ...
        <form role="form"
              action="{{ route('faqs.update', $faq->id) }}" method="POST">
            <div class="card-body p-0">
                ...
                <div class="form-group row mt-3 {{ $errors->has('title') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-8">
                       <textarea id="title" class="form-control" name="title">{{$faq->title}}</textarea>
                        @if ($errors->has('title'))
                ...
                <div class="editor form-group row mt-3 {{ $errors->has('data') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-8">
                        <textarea id="data" class="form-control" name="data">{{$faq->data}}</textarea>
                        @if ($errors->has('data'))
                            <span class="form-text">
                        ...
                        <select class="form-control" name="order_no" id="order_no">
                            ...
                            @if ($total_faqs > 0)
                                @for($count = 2; $count <= $total_faqs + 1; $count++)
                                    <option <?php echo ($count == $faq->order_no) ? 'selected="selected"' : ''; ?> value="{{$count}}">{{$count}}</option>
                                @endfor
                            @endif
                        </select>
                ...
                <div class="form-group row{{ $errors->has('is_for_student') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-6 radio">
                        <label for="is_for_student" class="col-md-3"> <input
                                    {{ $faq->is_for_student ? 'checked="checked"' : ''}} type="checkbox"
                ...
                <div class="form-group row{{ $errors->has('is_for_teacher') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-6 radio">
                        <label for="is_for_teacher" class="col-md-3"> <input
                                    {{ $faq->is_for_teacher ? 'checked="checked"' : ''}} type="checkbox"
