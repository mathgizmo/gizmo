...
@section('content')
    ...
    @endif
    <div class="card">
        <div class="card-header font-weight-bold d-flex flex-row">Create Tutorial</div>
        <form role="form" action="{{ route('tutorials.store') }}" method="POST">
                        ...
                        <select class="form-control" name="order_no" id="order_no">
                            ...
                            @if ($total_tutorials > 0)
                                @for($count = 2; $count <= $total_tutorials + 1; $count++)
                                    <option <?php echo ($count > $total_tutorials) ? 'selected="selected"' : ''; ?> value="{{$count}}">{{$count}}</option>
                                @endfor
                            @endif
                        </select>
            ...
            <div class="card-footer">
                <a class="btn btn-secondary" href="{{ route('tutorials.index') }}">Back</a>
