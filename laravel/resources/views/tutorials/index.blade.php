...
@section('content')
    ...
    <div class="card">
        <div class="card-header font-weight-bold d-flex flex-row justify-content-between">
            Manage Tutorials
            <a class="btn btn-dark btn-sm" href="{{ route('tutorials.create') }}">+ add tutorial</a>
        </div>
                ...
                <table class="table table-striped">
                    ...
                    <tr>
                        <th style="min-width: 60px;">
                            ID
                            <a href="{{ route('tutorials.index', array_merge(request()->all(), ['sort' => 'id', 'order' => ((request()->sort == 'id' && request()->order == 'desc') ? 'asc' : ((request()->sort == 'id' && request()->order == 'asc') ? '' : 'desc'))])) }}">
                                <i class="fa fa-fw fa-sort{{ (request()->sort == 'id' && request()->order == 'asc') ? '-up' : '' }}{{ (request()->sort == 'id' && request()->order == 'desc') ? '-down' : '' }}"></i>
                            </a>
                        </th>
                        <th style="min-width: 120px;">
                            Order No
                            <a href="{{ route('tutorials.index', array_merge(request()->all(), ['sort' => 'order_no', 'order' => ((request()->sort == 'order_no' && request()->order == 'desc') ? 'asc' : ((request()->sort == 'order_no' && request()->order == 'asc') ? '' : 'desc'))])) }}">
                                <i class="fa fa-fw fa-sort{{ (request()->sort == 'order_no' && request()->order == 'asc') ? '-up' : '' }}{{ (request()->sort == 'order_no' && request()->order == 'desc') ? '-down' : '' }}"></i>
                            </a>
                        </th>
                        <th style="min-width: 180px;">
                            Title
                            <a href="{{ route('tutorials.index', array_merge(request()->all(), ['sort' => 'title', 'order' => ((request()->sort == 'title' && request()->order == 'desc') ? 'asc' : ((request()->sort == 'title' && request()->order == 'asc') ? '' : 'desc'))])) }}">
                    ...
                    @foreach($tutorials as $tutorial)
                        <tr>
                            <td>{{$tutorial->id}}</td>
                            <td>{{$tutorial->order_no}}</td>
                            <td>{{$tutorial->title}}</td>
                            <td>
                                @if($tutorial->is_for_student)
                                    <badge class="badge badge-primary">Student</badge>
                                @endif
                                @if($tutorial->is_for_teacher)
                                    <badge class="badge badge-dark">Teacher</badge>
                                @endif
                            </td>
                            <td class="text-right">
                                <a class="btn btn-dark" href="{{ route('tutorials.edit', $tutorial->id) }}">Edit</a>
                                <form action="{{ route('tutorials.destroy', $tutorial->id) }}"
