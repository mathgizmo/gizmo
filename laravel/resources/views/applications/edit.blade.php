                ...
                <div class="form-group row mt-3 {{ $errors->has('tag_id') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-8">
                        <select id="tag_id" name="tag_id[]" class="form-control" style="max-width: 260px;" multiple>
                            @php($oldTagIds = old('tag_id', $selected_tag_ids ?? []))
                            @foreach(\App\Tag::orderBy('name')->get() as $tag)
                                <option value="{{ $tag->id }}" {{ in_array($tag->id, $oldTagIds ?? []) ? 'selected' : '' }}>
                                    {{ $tag->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <small class="form-text text-muted">Filtering: only Units with at least one selected Tag remain enabled.</small>
                    </div>
                </div>
